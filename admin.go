...
func getJWTTokenFromSecrets(ctx context.Context, secrets k8sutil.SecretInterface, name string) (connection.Authentication, error) {
...
func getCACertificate(ctx context.Context, secrets k8sutil.SecretInterface, name string) (*x509.CertPool, error) {
	ctxChild, cancel := context.WithTimeout(ctx, k8sutil.GetRequestTimeout())
	defer cancel()
